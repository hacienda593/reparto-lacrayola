...
- `[ ]` **4. Checkout de Facturación y Cierre de Caja (Shopper)**
  - `[ ]` Diseñar el formulario/modal de checkout en `/picking/[id]` al completar la compra.
  - `[ ]` Implementar campos: número factura SRI, total real, IVA y captura de foto de factura.
  - `[ ]` Guardar la imagen de factura en Storage y guardar el registro de compra en `rep_facturas_compras`.
  - `[ ]` Actualizar el estado de asignación a `recolectado` únicamente tras finalizar este checkout.
...
- `[ ]` **6. Bloqueo por Exceso de Efectivo en Mano (Rider)**
  - `[ ]` Programar el control de saldo de `efectivo_en_mano` comparándolo contra el límite de la configuración (ej. $100.00).
  - `[ ]` Renderizar la pantalla de bloqueo total con advertencia administrativa en `/repartidor` si excede el límite.
