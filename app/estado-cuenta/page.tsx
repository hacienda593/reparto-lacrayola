...
import { AlertTriangle,ArrowDownLeft,ArrowUpRight,Banknote,CalendarDays,Check,Loader2,RefreshCw,Scale,WalletCards,X } from 'lucide-react'
...
type Periodo={id:string;repartidor_id:string;desde:string;hasta:string;ganancias:number;caja_custodia:number;posicion_neta:number;monto_pagar:number;monto_cobrar:number;estado:string;banco_pago:string|null;referencia_pago:string|null;comprobante_pago_path:string|null;pagado_at:string|null}
...
export default function EstadoCuentaPage(){
 ...
 // Atajos de rango -- llenar las fechas a mano cada semana es el paso que
 // más se olvida hacer bien (ej. dejar un día afuera sin querer).
 function irA(rango:'semanaPasada'|'estaSemana'|'ultimos7'){
  const h=new Date()
  if(rango==='ultimos7'){const d=new Date(h);d.setDate(h.getDate()-6);setDesde(iso(d));setHasta(iso(h));return}
  const lunesDe=(base:Date)=>{const l=new Date(base);l.setDate(base.getDate()-((base.getDay()+6)%7));return l}
  const l=rango==='estaSemana'?lunesDe(h):(()=>{const ls=lunesDe(h);ls.setDate(ls.getDate()-7);return ls})()
  const dgo=new Date(l);dgo.setDate(l.getDate()+6)
  setDesde(iso(l));setHasta(iso(dgo))
 }
 ...
 // Hallazgo real: cerrar un período solo calculaba cuánto se le debe pagar
 // al repartidor -- nunca quedaba registro de que ese pago (transferencia/
 // depósito a SU cuenta) realmente se hizo, ni banco, ni referencia, ni
 // comprobante. Espejo de rep_depositos_repartidor, que sí exige todo eso
 // cuando el dinero va al revés (repartidor -> empresa).
 const [pagoModal,setPagoModal]=useState<Periodo|null>(null)
 const [pagoBanco,setPagoBanco]=useState(''),[pagoRef,setPagoRef]=useState(''),[pagoFile,setPagoFile]=useState<File|null>(null),[pagando,setPagando]=useState(false)
 function abrirPago(p:Periodo){setPagoModal(p);setPagoBanco('');setPagoRef('');setPagoFile(null)}
 async function confirmarPago(){
  if(!pagoModal)return
  if(!pagoBanco.trim()||!pagoRef.trim()){setError('Banco y referencia son obligatorios para registrar el pago');return}
  setPagando(true);setError('')
  let comprobantePath:string|null=null
  if(pagoFile){
   const nombreArchivo=`pagos-colaborador/${pagoModal.id}_${Date.now()}.${pagoFile.name.split('.').pop()}`
   const {error:errSubida}=await supabase.storage.from('comprobantes-proveedores').upload(nombreArchivo,pagoFile)
   if(errSubida){setError('No se pudo subir el comprobante: '+errSubida.message);setPagando(false);return}
   comprobantePath=nombreArchivo
  }
  const {error}=await supabase.rpc('registrar_pago_periodo',{p_periodo_id:pagoModal.id,p_banco:pagoBanco.trim(),p_referencia:pagoRef.trim(),p_comprobante_path:comprobantePath,p_request_id:crypto.randomUUID()})
  setPagando(false)
 ...
 return <div className="flex min-h-screen bg-slate-100"><Sidebar/><main className="flex-1 space-y-5 p-4 pt-16 md:ml-56 md:p-6">
  ...
  <div className="grid gap-4 xl:grid-cols-[300px_1fr]"><aside className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm"><p className="px-2 pb-2 text-[10px] font-black uppercase tracking-wider text-slate-400">Colaboradores</p>{loading?<Loader2 className="m-6 animate-spin"/>:<div className="space-y-1">{estados.map(e=><button key={e.repartidor_id} onClick={()=>setSeleccion(e.repartidor_id)} className={`w-full rounded-xl p-3 text-left ${seleccion===e.repartidor_id?'bg-slate-900 text-white':'hover:bg-slate-50'}`}><div className="flex justify-between gap-2"><span className="truncate text-sm font-bold">{e.nombre}</span><span className={`text-xs font-black ${e.posicion_neta>=0?'text-green-500':'text-orange-500'}`}>{money(e.posicion_neta)}</span></div><p className={`text-[10px] ${seleccion===e.repartidor_id?'text-slate-400':'text-slate-500'}`}>{e.movimientos} movimientos</p></button>)}</div>}</aside>
   ...
   <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><h2 className="font-black">Cierres anteriores de {actual.nombre}</h2><div className="mt-3 space-y-2">{periodos.filter(p=>p.repartidor_id===seleccion).map(p=><div key={p.id} className="rounded-xl bg-slate-50 p-3 text-xs space-y-1.5"><div className="flex items-center justify-between"><span className="font-bold">{p.desde} → {p.hasta}</span><span className={p.posicion_neta>=0?'text-green-700':'text-orange-700'}>{p.posicion_neta>=0?`Pagar ${money(p.monto_pagar)}`:`Cobrar ${money(p.monto_cobrar)}`}</span><span className="rounded-full bg-white px-2 py-1 font-bold capitalize">{p.estado}</span></div>{p.estado==='cerrado'&&p.monto_pagar>0&&<button onClick={()=>abrirPago(p)} className="w-full rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-bold py-1.5 text-[10px]">Registrar pago (banco, referencia, comprobante)</button>}{p.estado==='pagado'&&<p className="text-[10px] text-slate-500">Pagado {p.pagado_at?new Date(p.pagado_at).toLocaleDateString('es-EC'):''} · {p.banco_pago} · Ref. {p.referencia_pago}</p>}</div>)}{!periodos.filter(p=>p.repartidor_id===seleccion).length&&<p className="py-3 text-center text-xs text-slate-400">Sin cierres todavía.</p>}</div></div></>}
   {pagoModal&&<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4"><div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-2xl space-y-3"><div className="flex justify-between items-start"><div><h2 className="font-black">Registrar pago al colaborador</h2><p className="text-xs text-slate-500">{pagoModal.desde} → {pagoModal.hasta} · {money(pagoModal.monto_pagar)}</p></div><button onClick={()=>setPagoModal(null)}><X size={18}/></button></div><label className="block text-xs font-bold text-slate-600">Banco / medio *<input value={pagoBanco} onChange={e=>setPagoBanco(e.target.value)} placeholder="Ej. Banco Pichincha" className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm"/></label><label className="block text-xs font-bold text-slate-600">Número de referencia *<input value={pagoRef} onChange={e=>setPagoRef(e.target.value)} placeholder="Nro. de comprobante" className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm"/></label><label className="block text-xs font-bold text-slate-600">Foto del comprobante<input type="file" accept="image/*" onChange={e=>setPagoFile(e.target.files?.[0]??null)} className="mt-1 w-full text-xs"/></label><button onClick={confirmarPago} disabled={pagando} className="w-full flex items-center justify-center gap-2 rounded-xl bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white font-bold py-3 text-sm"><Check size={15}/>{pagando?'Guardando…':'Confirmar pago registrado'}</button></div></div>}
