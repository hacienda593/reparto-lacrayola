...
export async function POST(req:NextRequest){
 try{
  ...
  const{clave,asignacionId}=await req.json();if(!asignacionId)return NextResponse.json({error:'Asignación requerida'},{status:400})
  const{data:a,error}=await supabase.from('rep_asignaciones').select('id,pedido_id').eq('id',asignacionId).single();if(error||!a)return NextResponse.json({error:'No tiene acceso a esta asignación'},{status:403})
