...
SELECT p.id,p.numero,p.created_at,p.nombre_cliente,p.telefono,p.total,p.estado pedido_estado,
 ...
 COALESCE(pk.total,oi.total,0) items_total,
 COALESCE(pk.resueltos,oi.resueltos,0) items_resueltos,
 COALESCE(pk.faltantes,oi.faltantes,0) items_faltantes,
 ...
 CASE
  ...
  WHEN COALESCE(pk.pendientes,oi.pendientes,0)>0 THEN 'completar_compra'
  WHEN COALESCE(pk.faltantes,oi.faltantes,0)>0 THEN 'revisar_faltantes'
 ...
 ARRAY_REMOVE(ARRAY[
  ...
  CASE WHEN COALESCE(pk.pendientes,oi.pendientes,0)>0 THEN 'Items pendientes' END,
  CASE WHEN COALESCE(pk.faltantes,oi.faltantes,0)>0 THEN 'Items faltantes' END,
